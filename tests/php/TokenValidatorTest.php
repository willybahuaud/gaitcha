...
class TokenValidatorTest extends TestCase
{
    ...
    {
        $this->config    = new Config(['secret' => 'test-secret-key', 'ttl' => 120]);
    ...
    {
        $now   = 1700000000;
        $token = $this->generator->generateToken('_gc_test1234', $now);

        $result = $this->validator->validate($token, $now);

        $this->assertTrue($result['valid']);
        $this->assertSame('_gc_test1234', $result['field_name']);
    ...
    {
        $now   = 1700000000;
        $token = $this->generator->generateToken('_gc_test', $now);
    ...
    {
        $token = '_gc_test.1700000000.forgedsignatureforgedsignatureforgedsignatureforgedsignature';
    ...
    {
        $otherConfig    = new Config(['secret' => 'wrong-secret']);
        $otherGenerator = new TokenGenerator($otherConfig);

        $token  = $otherGenerator->generateToken('_gc_test', 1700000000);
        $result = $this->validator->validate($token, 1700000000);
    ...
    {
        $result = $this->validator->validate('_gc_test.notanumber.sig', 1700000000);
    ...
    {
        $now   = 1700000000;
        $token = $this->generator->generateToken('_gc_test', $now + 100);
